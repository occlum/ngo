...
impl FileHandle {
    ...
    /// Create a file handle for an timer fd.
    pub fn new_timer(file: TimerFd) -> Self {
    ...
    pub fn as_timer_fd(&self) -> Option<&TimerFd> {
        match &self.0.file {
            AnyFile::Timer(timer_fd) => Some(timer_fd),
            _ => None,
        }
    }
