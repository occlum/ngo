...
pub struct AsyncMountFS {
    ...
    self_mountpoint: Option<Arc<MInode>>,
...
impl AsyncMountFS {
    ...
    async fn mountpoint_root_inode(&self) -> Arc<MInode> {
        MInode::new(
...
impl MInode {
    ...
    async fn overlaid_inode(&self) -> Arc<MInode> {
    ...
    pub async fn find(&self, name: &str) -> Result<Arc<Self>> {
        let mut result = self.self_ref.upgrade().unwrap();
        ...
        loop {
            match name {
                ...
                ".." => {
                    // Going Up
                    if result.is_mountpoint_root().await {
                        // May trespass filesystem border
                        match &result.vfs.self_mountpoint {
                            Some(inode) => {
                                result = inode.clone();
                            }
                            None => break,
                        ...
                        result = Self::new(result.inner.find("..").await?, result.vfs.clone());
                        break;
                    }
                }
                _ => {
                    ...
                    // May trespass filesystem border
                    result = Self::new(
                        result.overlaid_inode().await.inner.find(name).await?,
                        result.vfs.clone(),
                    )
                    .overlaid_inode()
                    .await;
